...
/// Per spec 03 Section 3.2.2: Tickets transition PENDING -> LIVE -> EXPIRED/SPENT
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum TicketStatus {
    /// Ticket is pending finality (not yet eligible for voting)
    /// Per spec: Ticket transitions to LIVE when block reaches POS_FINALITY_DEPTH
    Pending,
    /// Ticket is live and eligible for voter selection
    Live,
    /// Ticket has been used to vote
    Voted,
    /// Ticket has expired (no longer eligible for voting)
    Expired,
    /// Ticket has been revoked (e.g., due to slashing)
    Revoked,
}
...
pub enum Transaction {
    ...
    /// PoS ticket slashing for non-participation
    TicketSlashNonParticipation {
        version: u32,
        inputs: Vec<TxInput>,
        outputs: Vec<TxOutput>,
        ticket_id: [u8; 32],
        proof: TicketNonParticipationProof,
        lock_time: u32,
        fee: u64,
        witness: Vec<Vec<u8>>,
    },
    /// PoS ticket slashing for malicious behavior (double-voting, invalid votes)
    TicketSlashMalicious {
        version: u32,
        inputs: Vec<TxInput>,
        outputs: Vec<TxOutput>,
        ticket_id: [u8; 32],
        proof: TicketMaliciousProof,
        lock_time: u32,
        fee: u64,
        witness: Vec<Vec<u8>>,
    },
...
impl Transaction {
    ...
    pub fn get_inputs(&self) -> &[TxInput] {
        match self {
            ...
            Transaction::TicketSlashNonParticipation { inputs, .. } => inputs,
            Transaction::TicketSlashMalicious { inputs, .. } => inputs,
            Transaction::GovernanceProposal(proposal) => &proposal.inputs,
            Transaction::GovernanceVote(vote) => &vote.inputs,
            Transaction::MasternodeSlashTx(tx) => &tx.inputs,
        }
    }
    ...
    pub fn get_outputs(&self) -> &[TxOutput] {
        match self {
            ...
            Transaction::TicketSlashNonParticipation { outputs, .. } => outputs,
            Transaction::TicketSlashMalicious { outputs, .. } => outputs,
            Transaction::GovernanceProposal(proposal) => &proposal.outputs,
            Transaction::GovernanceVote(vote) => &vote.outputs,
            Transaction::MasternodeSlashTx(tx) => &tx.outputs,
        }
    }

    /// Returns the lock_time for the transaction.
    ///
    ...
    pub fn get_lock_time(&self) -> u32 {
        match self {
            ...
            Transaction::TicketSlashNonParticipation { lock_time, .. } => *lock_time,
            Transaction::TicketSlashMalicious { lock_time, .. } => *lock_time,
    ...
    pub fn get_fee(&self) -> u64 {
        match self {
            ...
            Transaction::TicketSlashNonParticipation { fee, .. } => *fee,
            Transaction::TicketSlashMalicious { fee, .. } => *fee,
    ...
    pub fn get_witnesses(&self) -> &[Vec<u8>] {
        match self {
            ...
            Transaction::TicketSlashNonParticipation { witness, .. } => witness,
            Transaction::TicketSlashMalicious { witness, .. } => witness,
        }
    }
    ...
    pub fn get_witnesses_mut(&mut self) -> Result<&mut Vec<Vec<u8>>, TransactionError> {
        match self {
            Transaction::Standard { witness, .. } => Ok(witness),
            Transaction::Coinbase { witness, .. } => Ok(witness),
            Transaction::MasternodeRegister { witness, .. } => Ok(witness),
            Transaction::MasternodeCollateral { witness, .. } => Ok(witness),
            Transaction::ActivateProposal { witness, .. } => Ok(witness),
            Transaction::TicketPurchase { witness, .. } => Ok(witness),
            Transaction::TicketRedemption { witness, .. } => Ok(witness),
            ...
            Transaction::GovernanceProposal(_) => Err(TransactionError::UnsupportedWitnessOperation(
                "Cannot get mutable witnesses for GovernanceProposal".to_string(),
            )),
            Transaction::GovernanceVote(_) => Err(TransactionError::UnsupportedWitnessOperation(
                "Cannot get mutable witnesses for GovernanceVote".to_string(),
            )),
            Transaction::MasternodeSlashTx(tx) => Ok(&mut tx.witness),
            Transaction::TicketSlashNonParticipation { witness, .. } => Ok(witness),
            Transaction::TicketSlashMalicious { witness, .. } => Ok(witness),
        }
    }
    ...
    pub fn set_witnesses(&mut self, witnesses: Vec<Vec<u8>>) {
        match self {
            ...
            Self::TicketSlashNonParticipation { witness, .. } => *witness = witnesses,
            Self::TicketSlashMalicious { witness, .. } => *witness = witnesses,
        }
    }
...
impl MasternodeList {
    ...
    pub fn register_masternode(
        ...
        let entry = MasternodeEntry {
            ...
            collateral_amount: 0, // Assuming collateral_amount is not provided in registration
        };
    ...
    /// Get all masternodes in the list
    pub fn get_all_masternodes(&self) -> &HashMap<MasternodeID, MasternodeEntry> {
        &self.map
    }
    ...
    pub fn select_masternodes_for_quorum(
        &self,
        count: usize,
        seed: Option<u64>,
    ) -> Vec<&MasternodeEntry> {
        // Implements deterministic, pseudo-random, and uptime-based selection per protocol spec.
        ...
        let mut candidates: Vec<&MasternodeEntry> = self
            ...
            .filter(|mn| mn.status == MasternodeStatus::Active)
            .collect();
        // Sort by uptime (descending), then by collateral outpoint for determinism
        candidates.sort_by(|a, b| {
            b.last_successful_pose_height
                ...
                .then_with(|| {
                    a.identity
                        .collateral_outpoint
                        .cmp(&b.identity.collateral_outpoint)
                })
        });
        // If a seed is provided, shuffle deterministically for pseudo-random selection
    ...
    pub fn distribute_rewards(&mut self, block_height: u32, total_block_reward: u64) {
        ...
        for mn_id in active_masternodes {
            ...
            println!(
                "Masternode {:?} received {} satoshis at height {}",
...
/// Represents a witness signature for slashing proofs.
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
...
/// Represents a Masternode non-participation proof for slashing.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Represents a PoS ticket non-participation proof for slashing.
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct TicketNonParticipationProof {
    /// The ID of the ticket being slashed.
    pub ticket_id: [u8; 32],
    /// The block height at which the non-participation was detected.
    pub detection_block_height: u64,
    /// The block hash for which the ticket was selected to vote but failed to participate.
    pub target_block_hash: [u8; 32],
    /// The block height where the ticket was selected for voting.
    pub selection_block_height: u64,
    /// Cryptographic proof that the ticket was selected for voting (DPRF output).
    pub selection_proof: Vec<u8>,
    /// Signatures from witness nodes attesting to the non-participation.
    pub witness_signatures: Vec<WitnessSignature>,
}

/// Represents a PoS ticket malicious behavior proof for slashing.
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct TicketMaliciousProof {
    /// The ID of the ticket being slashed.
    pub ticket_id: [u8; 32],
    /// The block height at which the malicious behavior was detected.
    pub detection_block_height: u64,
    /// The type of malicious behavior (double-voting, invalid vote, etc.).
    pub malicious_action_type: TicketMaliciousActionType,
    /// The conflicting signatures or invalid vote data.
    pub proof_data: Vec<u8>,
    /// Signatures from witness nodes attesting to the malicious behavior.
    pub witness_signatures: Vec<WitnessSignature>,
}

/// Represents the type of malicious behavior for PoS ticket slashing.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub enum TicketMaliciousActionType {
    /// Ticket signed two conflicting votes for the same block height.
    DoubleVoting,
    /// Ticket signed a vote for an invalid block.
    InvalidVote,
    /// Ticket signed a vote with an invalid signature.
    InvalidSignature,
}
...
impl Block {
    ...
    /// Calculates the merkle root of the transactions in this block.
    pub fn calculate_merkle_root(&self) -> [u8; 32] {
        Self::calculate_merkle_root_from_transactions(&self.transactions)
    }

    /// Calculates the merkle root from a list of transactions.
    pub fn calculate_merkle_root_from_transactions(transactions: &[Transaction]) -> [u8; 32] {
    ...
    pub fn calculate_merkle_root_from_hashes(hashes: &[[u8; 32]]) -> [u8; 32] {
        if hashes.is_empty() {
            return [0u8; 32];
        }
        ...
        while current_level.len() > 1 {
            ...
            for chunk in current_level.chunks(2) {
                ...
                // Hash the concatenated left and right hashes
...
pub struct ConsensusParams {
    ...
    /// Current block reward (in satoshis)
    pub block_reward: u64,
    /// Coinbase maturity period (in blocks)
    pub coinbase_maturity: u32,
    /// Minimum value for a transaction output to not be considered dust (in satoshis)
    pub dust_limit: u64,
    ...
    pub masternode_collateral_amount: u64,

    /// Maximum blocks of inactivity before masternode is considered inactive
    pub max_inactivity_blocks: u32,
...
impl Default for ConsensusParams {
    fn default() -> Self {
        ConsensusParams {
            // PoS Ticket Parameters (from protocol constants)
            min_stake: 1_00000000,                // 1 RUST minimum stake
            max_tickets_per_stake: 1,             // Only 1 ticket per stake for simplicity
            ticket_price: 100_00000000, // 100 RUST initial price (from protocol constants)
            ticket_maturity: 10,        // 10 blocks
            ticket_expiry: 4096,        // ~7 days (from protocol constants)
            tickets_per_round: 5,       // VOTERS_PER_BLOCK from protocol constants
            min_block_time: 150,        // 2.5 minutes in seconds
            reward_amount: 1_000_000,   // 0.01 RUST per ticket
            target_live_tickets: 20000, // From protocol constants
            price_adjustment_factor: 0.05, // TICKET_PRICE_ADJUSTMENT_K_P from protocol constants
            ...
            initial_block_reward: 50_000_000_000, // 500 RUST
            pos_reward_ratio: 0.6,      // 60% of total reward to PoS stakers
            miner_address: vec![0u8; 20], // Placeholder
            ticket_price_adjustment_period: 2016, // From protocol constants (~3.5 days)
            max_ticket_price: 10000_00000000, // 10,000 RUST max (from protocol constants)
            min_ticket_price: 1_00000000, // 1 RUST min (from protocol constants)

            // Governance parameters (from protocol constants)
            proposal_stake_amount: 1000_00000000, // 1000 RUST (from protocol constants)
            voting_period_blocks: 20160, // Approximately 2 weeks (14 days * 1440 blocks/day)
            pos_voting_quorum_percentage: 0.20, // 20% from protocol constants
            mn_voting_quorum_percentage: 0.50, // 50% from protocol constants
            pos_approval_percentage: 0.75, // 75% from protocol constants
            mn_approval_percentage: 0.66, // 66% from protocol constants
            activation_delay_blocks: 100, // From protocol constants
            grace_period_blocks: 10,     // From protocol constants
            slash_forgiveness_period: 100, // From protocol constants
            malicious_behavior_slash_percentage: 1.0, // 100% from protocol constants

            // Masternode Proof-of-Service parameters (from protocol constants)
            pose_challenge_period_blocks: 60, // From protocol constants (~2.5 hours)
            ...
            pose_response_timeout_seconds: 60, // From protocol constants
            max_pose_failures: 3,             // From protocol constants
            reset_failures_period: 100,       // From protocol constants
            min_witness_signatures: 3,        // Minimum 3 witness signatures for a valid proof
            pos_finality_depth: 1,            // From protocol constants
            max_block_size: 4_000_000,        // 4 MB
            max_tx_size: 100_000,             // 100 KB
            block_reward: 50_000_000_000,     // 500 RUST (same as initial_block_reward)
            coinbase_maturity: 100,           // From constants file
            dust_limit: 500,                  // From constants file
            min_relay_tx_fee: 1,              // From constants file (1 satoshi per byte)
            masternode_collateral_amount: 26000_00000000, // 26,000 RUST from protocol constants
            ...
            max_inactivity_blocks: 1000,               // From protocol constants
        }
    }
}
...
impl ConsensusParams {
    pub fn testnet() -> Self {
        ConsensusParams {
            // PoS Ticket Parameters (scaled down for testnet)
            min_stake: 1_000_000,     // 0.01 RUST minimum stake
            ticket_price: 10_000_000, // 0.1 RUST initial price
            ticket_maturity: 2,
            ticket_expiry: 144, // 1 day
            tickets_per_round: 2,
            min_block_time: 10,
            reward_amount: 100_000, // 0.001 RUST per ticket
            target_live_tickets: 100,
            difficulty_adjustment_window: 10,
            halving_interval: 1000,
            initial_block_reward: 1_000_000_000, // 10 RUST
            pos_reward_ratio: 0.8,
            ticket_price_adjustment_period: 10,
            max_ticket_price: 1_000_000,       // 0.01 RUST max
            min_ticket_price: 1_000,           // 0.00001 RUST min
            proposal_stake_amount: 10_000_000, // 0.1 RUST
            ...
            min_relay_tx_fee: 100,                       // 100 satoshis per byte
            masternode_collateral_amount: 1_000_000_000, // 10 RUST
            max_inactivity_blocks: 1000,                 // From protocol constants
            ..Default::default()
        }
    }
    ...
    pub fn regtest() -> Self {
        ConsensusParams {
            // PoS Ticket Parameters (minimal for regtest)
            min_stake: 100,
            ...
            min_ticket_price: 1,
            proposal_stake_amount: 1000,
            voting_period_blocks: 10,
            activation_delay_blocks: 5,
            grace_period_blocks: 1,
            slash_forgiveness_period: 5,
            malicious_behavior_slash_percentage: 0.5,
            pose_challenge_period_blocks: 2,
            num_pose_challengers: 1,
            ...
            min_witness_signatures: 1,
            pos_finality_depth: 2,
            max_block_size: 100_000,               // 100 KB
            min_relay_tx_fee: 10,                  // 10 satoshis per byte
            masternode_collateral_amount: 100_000, // 0.001 RUST
            max_inactivity_blocks: 1000,           // From protocol constants
            ..Default::default()
        }
    }
