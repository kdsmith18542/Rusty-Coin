...
mod p2p_serialization_tests {
    ...
    fn test_p2p_message_ping_pong() {
        ...
        let ping_deserialized: P2PMessage = bincode::deserialize(&ping_serialized).unwrap();
        assert_eq!(ping, ping_deserialized);

        let pong_serialized = bincode::serialize(&pong).unwrap();
        let pong_deserialized: P2PMessage = bincode::deserialize(&pong_serialized).unwrap();
