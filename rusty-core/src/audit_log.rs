...
use rusty_shared_types::masternode::{MasternodeID, MasternodeIdentity};
...
pub fn log_node_startup(node_id: &str, listen_port: u16) {
    event!(
        Level::INFO,
        "Node started with ID: {}, listening on port {}",
        node_id,
        listen_port
    );
}
...
pub fn log_block_validation_failed(block: &BlockHeader, error: &ConsensusError) {
    event!(
        Level::WARN,
        "Block validation failed for block height={}, hash={:?}: {}",
        block.height,
        block.hash(),
        error
    );
...
pub fn log_transaction_received(tx: &Transaction) {
    event!(
        Level::INFO,
        "New transaction added to mempool: txid={:?}",
        tx.txid()
    );
}
...
pub fn log_transaction_validation_failed(tx: &Transaction, error: &ConsensusError) {
    event!(
        Level::WARN,
        "Transaction validation failed for txid={:?}: {}",
        tx.txid(),
        error
    );
}
...
pub fn log_masternode_registered(identity: &MasternodeIdentity) {
    event!(
        Level::INFO,
        "Masternode registered: id={:?}, address={}",
        identity.collateral_outpoint,
...
pub fn log_masternode_slashed(slash_tx: &rusty_shared_types::masternode::MasternodeSlashTx) {
    event!(
        Level::WARN,
        "Masternode slashed: masternode_id={:?}, reason={:?}",
        slash_tx.masternode_id,
        slash_tx.reason
    );
}
...
pub fn log_masternode_registration_failed(identity: &MasternodeIdentity, error: &ConsensusError) {
    event!(
        Level::WARN,
        "Masternode registration failed for id={:?}: {}",
        identity.collateral_outpoint,
        error
    );
}
...
pub fn log_governance_proposal_submitted(proposal: &GovernanceProposal) {
    event!(
        Level::INFO,
        "Governance proposal submitted: id={:?}, type={:?}, title={}",
...
pub fn log_governance_vote_cast(vote: &GovernanceVote) {
    event!(
        Level::INFO,
        "Governance vote cast: proposal_id={:?}, voter_id={:?}, choice={:?}",
        vote.proposal_id,
        vote.voter_id,
        vote.vote_choice
    );
...
pub fn log_governance_proposal_evaluated(
    ...
    event!(
        Level::INFO,
        "Governance proposal evaluated: proposal_id={}, outcome={:?}, outcome_reason={:?}",
...
#[tracing::instrument(level = "info", skip(proposal_id, outcome_reason))]
pub fn log_governance_proposal_resolved(proposal_id: &Hash, outcome_reason: &str) {
    event!(
        Level::INFO,
        "Governance proposal resolved: id={:?}, outcome={}",
        proposal_id,
        outcome_reason
    );
}
