...
impl LightClient {
    ...
    pub fn clear_cache(&mut self) {
        self.pro        info!("Lightof_cache.clear();
 client proof cache cleared");
    }
    ...
    async fn request_utxo_proof_from_network_with_retries(
        ...
        for attempt in 0..=self.config.max_retries {
            ...
            // Now make the request (permit is held but we don't need it anymore for the call)
            let result = self.request_utxo_proof_from_network(outpoint, block_height).await;
    ...
    async fn request_utxo_batch_proof_from_network_with_retries(
        ...
        for attempt in 0..=self.config.max_retries {
            ...
            let result = self.request_utxo_batch_proof_from_network(outpoints, block_height).await;
        ...
        Err(last_error.unwrap_or_else(|| 
            ConsensusError::NetworkError("All batch proof retry