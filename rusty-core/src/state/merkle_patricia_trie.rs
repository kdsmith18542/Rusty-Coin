...
impl MerklePatriciaTrie {
    ...
    pub fn generate_batch_proof(
        &self,
        keys: &[Vec<u8>],
    ) -> Result<BatchMerkleProof, ConsensusError> {
    ...
    pub fn generate_range_proof(
        ...
        start_key: &[u8],
        end_key: &[u8],
    ) -> Result<RangeProof, ConsensusError> {
        ...
        self.collect_range_proof_nodes(
            &self.root_hash,
            &start_nibbles,
            &end_nibbles,
            &mut range_nodes,
    ...
    pub fn verify_proof(
        ...
        let computed_root =
            Self::compute_root_from_proof(&nibbles, expected_value, &proof.proof_nodes)?;
    ...
    pub fn verify_range_proof(proof: &RangeProof) -> Result<bool, ConsensusError> {
        ...
        let computed_root =
            Self::compute_range_root_from_proof(&proof.proof_nodes, &proof.included_keys)?;
        ...
        match node {
            ...
            TrieNode::Leaf {
                ...
                if existing_key == key_nibbles {
                    ...
                    let new_branch_node = if common.len() == existing_key.len()
                        && common.len() == key_nibbles.len()
                    {
            ...
            TrieNode::Extension {
                ...
                if common.len() == common_prefix.len() {
                    // New key is suffix of extension or extends it
                    let remaining_key_for_next = &key_nibbles[common_prefix.len()..];
                    let new_next_hash =
                        self.insert_recursive(next_hash, remaining_key_for_next, value)?;
    ...
    fn delete_recursive(
        ...
        node_hash: Hash,
        key_nibbles: &[u8],
    ) -> Result<(Hash, bool), ConsensusError> {
        ...
        let new_node_option = match node {
            ...
            TrieNode::Extension {
                ...
                if key_nibbles.starts_with(&common_prefix) {
                    ...
                    let (new_child_hash, child_deleted) =
                        self.delete_recursive(next_hash, remaining_key)?;

                    if child_deleted {
                        if new_child_hash == rusty_shared_types::Hash::from(blake3::hash(&[])) {
                            // Child became empty, this extension node is now redundant.
                            was_deleted = true;
            ...
            TrieNode::Branch {
                ...
                if key_nibbles.is_empty() {
                    ...
                    if non_empty_children_count == 0 {
                        ...
                        let child_hash_to_simplify =
                            children.iter().filter_map(|c| c.clone()).next().unwrap();
                        ...
                        let new_simplified_node = match child_node_to_simplify {
                            TrieNode::Leaf { key_end, value } => TrieNode::Leaf {
                                key_end: [0].iter().chain(key_end.iter()).cloned().collect(),
                                value,
                            },
                            TrieNode::Extension {
                                common_prefix: ext_common_prefix,
                                next_hash: ext_next_hash,
                            } => TrieNode::Extension {
                                common_prefix: [0]
                                    .iter()
                                    .chain(ext_common_prefix.iter())
                                    .cloned()
                                    .collect(),
                                next_hash: ext_next_hash,
                            },
                    ...
                    if let Some(child_hash) = children[first_nibble] {
                        let remaining_key = &key_nibbles[1..];
                        let (_new_child_hash, child_deleted) =
                            self.delete_recursive(child_hash, remaining_key)?;
                            ...
                            } else if non_empty_children_count == 1 && current_node_value.is_none()
                            {
                                ...
                                let new_simplified_node = match child_node_to_simplify {
                                    ...
                                    TrieNode::Extension {
                                        ...
                                        common_prefix: [first_nibble as u8]
                                            .iter()
                                            .chain(ext_common_prefix.iter())
                                            .cloned()
                                            .collect(),
        ...
        if let TrieNode::Leaf { key_end, value } = proof_nodes.last().unwrap() {
            if key_end != remaining_key {
                return Err(ConsensusError::TrieError(
                    "Key mismatch in leaf".to_string(),
                ));
            }
            if expected_value.map(|v| v.to_vec()) != Some(value.clone()) {
                return Err(ConsensusError::TrieError(
                    "Value mismatch in leaf".to_string(),
                ));
            }
        }
            ...
            match node {
                TrieNode::Extension {
                    ...
                    if !remaining_key.starts_with(common_prefix) {
                        return Err(ConsensusError::TrieError(
                            "Extension key mismatch".to_string(),
                        ));
                    }
                    if *next_hash != current_hash {
                        return Err(ConsensusError::TrieError(
                            "Extension hash mismatch".to_string(),
                        ));
                    }
                ...
                TrieNode::Branch { children, value } => {
                    if remaining_key.is_empty() {
                        ...
                        if value.is_none() && expected_value.is_some() {
                            return Err(ConsensusError::TrieError(
                                "Branch value mismatch".to_string(),
                            ));
                        }
                    } else {
                        ...
                        if children[first_nibble] != Some(current_hash) {
                            return Err(ConsensusError::TrieError(
                                "Branch child mismatch".to_string(),
                            ));
                        }
                ...
                _ => {
                    return Err(ConsensusError::TrieError(
                        "Unexpected node type in proof".to_string(),
                    ));
                }
            }
        }
    ...
    fn compute_range_root_from_proof(
        ...
        // Rebuild the root hash from proof structure per docs/specs/05_utxo_model_spec.md
        // The proof contains a path from root to leaf - reconstruct root hash
        let mut current_hash = if !proof_nodes.is_empty() {
            proof_nodes[proof_nodes.len() - 1].hash() // Start from leaf
        } else {
            return Ok([0u8; 32]);
        };

        // Work backwards through proof nodes to reconstruct root
        for i in (0..proof_nodes.len().saturating_sub(1)).rev() {
            let node = &proof_nodes[i];
            match node {
                TrieNode::Branch { children, value: _ } => {
                    // Find which child slot contains our path
                    let mut combined_data = Vec::new();
                    for child in children.iter() {
                        if let Some(child_hash) = child {
                            combined_data.extend_from_slice(child_hash);
                        } else {
                            combined_data.extend_from_slice(&[0u8; 32]);
                        }
                    }
                    current_hash = blake3::hash(&combined_data).into();
                }
                TrieNode::Extension {
                    common_prefix,
                    next_hash: _,
                } => {
                    // Combine prefix with child hash
                    let mut combined_data = Vec::new();
                    combined_data.extend_from_slice(common_prefix);
                ...
                TrieNode::Leaf { key_end, value } => {
                    ...
                    let mut combined_data = Vec::new();
                    combined_data.extend_from_slice(key_end);
    ...
    /// Public iterator over all trie nodes (hash, node)
    pub fn iter_nodes(&self) -> impl Iterator<Item = (&Hash, &TrieNode)> {
        self.nodes.iter()
    }
    ...
    fn encode_utxo_value(utxo: &Utxo) -> Result<Vec<u8>, ConsensusError> {
        bincode::serialize(utxo).map_err(|e| ConsensusError::SerializationError(e.to_string()))
    }
