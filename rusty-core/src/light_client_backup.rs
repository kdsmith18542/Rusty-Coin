...
impl LightClient {
    ...
    pub fn new(config: LightClientConfig, p2p_network: Arc<dyn LightClientP2PInterface>) -> Self {
        Self {
            ...
            concurrent_requests: tokio::sync::Semaphore::new(config.max_concurrent_requests),
        }
    }
    ...
    // Network request methods with proper P2P integration
    ...
    async fn request_utxo_proof_from_network_with_retries(
        ...
        for attempt in 0..=self.config.max_retries {
            // Drop the permit before calling the network request method
            let result = {
                let _permit = self.concurrent_requests.acquire().await.map_err(|e|
                    ConsensusError::NetworkError(format!("Failed to acquire semaphore: {}", e))
                )?;
                self.request_utxo_proof_from_network(outpoint, block_height).await
            };
    ...
    async fn request_utxo_batch_proof_from_network_with_retries(
        ...
        for attempt in 0..=self.config.max_retries {
            let result = {
                ...
                self.request_utxo_batch_proof_from_network(outpoints, block_height).await
            };
    ...
    async fn request_ticket_proof_from_network_with_retries(
        ...
        for attempt in 0..=self.config.max_retries {
            let result = {
                ...
                self.request_ticket_proof_from_network(ticket_id, block_height).await
            };
    ...
    async fn request_masternode_proof_from_network_with_retries(
        ...
        for attempt in 0..=self.config.max_retries {
            let result = {
                ...
                self.request_masternode_proof_from_network(masternode_key, block_height).await
            };
    ...
    async fn request_governance_proof_from_network_with_retries(
        ...
        for attempt in 0..=self.config.max_retries {
            let result = {
                ...
                self.request_governance_proof_from_network(proposal_key, block_height).await
            };
