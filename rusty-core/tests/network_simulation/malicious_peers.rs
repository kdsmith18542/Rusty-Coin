...
fn test_malformed_message_handling() {
    ...
    for (i, msg) in malformed_messages.iter().enumerate() {
        ...
        assert!(
            msg.len() >= 0,
            "Message should be processable (even if rejected)"
        );
    }
}
...
fn test_invalid_block_rejection() {
    ...
    assert!(
        invalid_block.header.difficulty_target > 0,
        "Block should have difficulty"
    );
}
