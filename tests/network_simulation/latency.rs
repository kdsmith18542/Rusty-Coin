...
fn test_block_propagation_with_latency() {
    ...
    for latency_ms in latencies {
        ...
        assert!(elapsed.as_millis() >= latency_ms as u128 - 10, 
...
fn test_network_partition() {
    ...
    for i in 0..blocks_in_partition {
        ...
        assert_ne!(block_a.header.previous_block_hash, 
                  block_b.header.previous_block_hash,
                  "Partitioned groups should produce different chains");
    }
    
    println!("Simulated network partition with {} nodes in each group", 
...
fn test_reorganization_after_partition() {
    ...
    println!("After partition healing, chain with {} blocks is selected", 
             longer_chain.len());
...
fn test_message_delivery_high_latency() {
    ...
    println!("Sent {} messages with {}ms latency each", num_messages, high_latency_ms);
    println!("Total time: {:?}, Expected: {:?}", elapsed, expected_time);
    
    // Allow some variance for system scheduling
    assert!(elapsed >= expected_time - Duration::from_millis(100),
