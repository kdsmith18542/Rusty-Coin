...
fn test_chain_split() {
    ...
    let partition_a_blocks = 5;
    let partition_b_blocks = 3;
    ...
    println!("Chain split: A has {} blocks, B has {} blocks", 
             chain_a.len(), chain_b.len());
    println!("Winning chain has {} blocks", winning_chain.len());
    
    assert_eq!(winning_chain.len(), partition_a_blocks, 
...
fn test_reorganization_depth_limits() {
    ...
    if attempted_reorg_depth > max_reorg_depth {
        println!("Reorganization depth {} exceeds limit {}, should be rejected",
                 attempted_reorg_depth, max_reorg_depth);
        // In a real implementation, this would be rejected
    }
    
    assert!(attempted_reorg_depth > max_reorg_depth, 
...
fn test_partition_healing() {
    ...
    println!("Partition healing: {} nodes in A, {} nodes in B",
             nodes_in_partition_a, nodes_in_partition_b);
    println!("After healing, all nodes should sync to chain with {} blocks",
             sync_target);
