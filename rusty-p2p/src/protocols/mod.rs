...
/// Protocol string for state proof synchronization protocol.
pub const PROOF_SYNC_PROTOCOL: &str = "/rusty/proof-sync/1.0";
...
pub fn supported_protocols() -> Vec<(StreamProtocol, ProtocolSupport)> {
    vec![
        ...
        (
            StreamProtocol::new(TX_PROPAGATION_PROTOCOL),
            ProtocolSupport::Full,
        ),
        (
